...
impl MediaResolver {
    ...
    pub async fn resolve(&self, input: &str) -> Result<ResolveResponse> {
        ...
        let (final_url, response) = self.fetch_follow(source.clone(), HeaderMap::new(), None).await?;
        ...
        if content_type.contains("html") || content_type.is_empty() {
            ...
            for candidate in html_video_candidates(&html, &final_url) {
                ...
                if let Ok((url, candidate_response)) = self
                    .fetch_follow(candidate, HeaderMap::new(), None)
                    .await
                {
    ...
    pub async fn stream(&self, id: &str, range: Option<&str>) -> Result<reqwest::Response> {
        ...
        self.fetch_follow(session.url, session.headers, range).await.map(|(_, r)| r)
    }
    ...
    async fn fetch_follow(
        ...
        for _ in 0..=MAX_REDIRECTS {
            ...
            let response = request.send().await.context("media server request failed")?;
...
fn html_video_candidates(html: &str, base: &Url) -> Vec<Url> {
    ...
    for tag in tags.find_iter(html) {
        ...
        let raw = match key.as_deref() {
            Some("og:video") | Some("og:video:url") | Some("og:video:secure_url")
            | Some("twitter:player:stream") => attrs.get("content"),
            _ if tag.as_str().to_ascii_lowercase().starts_with("<video")
                || tag.as_str().to_ascii_lowercase().starts_with("<source") => attrs.get("src"),
            _ => None,
        };
        if let Some(raw) = raw
...
fn html_title(html: &str) -> Option<String> {
    ...
    let title = decode_html_attr(raw).trim().chars().take(160).collect::<String>();
...
async fn validate_public_url(url: &Url) -> Result<Vec<SocketAddr>> {
    ...
    let host = url.host_str().ok_or_else(|| anyhow!("media URL has no host"))?;
    if host.eq_ignore_ascii_case("localhost") || host.ends_with(".local") {
        bail!("local-network media URLs are blocked")
    }
    let port = url.port_or_known_default().ok_or_else(|| anyhow!("unknown media URL port"))?;
